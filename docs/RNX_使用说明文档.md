...
#### 1. 功能概述
仪表配置界面用于管理系统中各类仪器的通信参数、SCPI命令集及性能规格，主要功能包括：
- **仪器参数配置**：设置IP地址、VISA资源字符串等连接参数
- **SCPI命令管理**：自定义各类仪器的控制命令集
- **性能规格设置**：定义仪器的频率/功率范围等硬件参数
- **配置导入导出**：支持JSON格式的配置备份与恢复
...
| 控件            | 说明                          |
|-----------------|-----------------------------|
| 仪器型号        | 设备型号标识（如PLASG-T8G40G）|
| 最小/最大频率   | 仪器支持的频率范围(Hz)        |
| 最小/最大功率   | 仪器支持的功率范围(dBm)       |

##### 2.2 命令配置选项卡
包含两个命令表格：
- **标准命令表**：基础控制命令（如频率设置、功率设置等）
- **调制命令表**：高级调制相关命令

支持命令的增删改操作，每行包含：
1. 命令名称（如`SET_FREQUENCY`）
2. SCPI命令字符串（如`FREQ:CW {value}Hz`）

##### 2.3 高级配置选项卡
预留扩展接口，用于特殊仪器的高级参数配置
...
##### 3.2 编辑命令集
1. 在命令表格中：
   - **添加命令**：点击"添加命令"，输入名称和SCPI字符串
   - **删除命令**：选中行后点击"移除命令"
2. SCPI命令支持参数占位符（如`{value}`）

##### 3.3 配置管理
| 按钮       | 功能                                                                 |
|------------|----------------------------------------------------------------------|
| 保存配置   | 保存当前所有修改到配置文件                                           |
| 重置更改   | 放弃未保存的修改，恢复到最后保存状态                                 |
| 导出配置   | 将当前配置另存为JSON文件（建议命名格式：`RNX_Config_[型号]_[日期].json`）|
| 导入配置   | 从JSON文件加载配置（自动校验格式）                                   |
...
```json
{
  "PLASG-T8G40G": {
    ...
    "modulation_commands": {
      "ENABLE_MOD": "MOD:STAT 1",
      "SET_MOD_TYPE": "MOD:TYPE {value}"
    }
  }
...
#### 5. 注意事项
1. 修改配置后需**手动保存**才会生效
2. 频率/功率范围设置会影响系统参数校验：
   - 若设置频率超出仪器范围，相关操作会被禁止
   - 功率补偿计算会参考配置的最大/最小功率值
3. 建议不同仪器型号使用独立的配置项
4. 移除仪器配置后，需要重新启动软件才能完全清除相关控制模块
5. 高级配置选项卡的功能需配合具体仪器型号使用

#### 6. 故障排查
| 现象                 | 可能原因                  | 解决方案                     |
|----------------------|--------------------------|----------------------------|
| 无法保存配置         | 配置文件被占用           | 检查是否有其他程序正在使用配置文件 |
| 导入配置失败         | JSON格式错误             | 使用文本编辑器检查文件语法     |
| 命令执行无响应       | SCPI命令语法错误         | 在仪器前面板手动验证命令有效性  |
| 新增仪器不显示       | 名称包含特殊字符         | 仅使用字母、数字和下划线命名   |
### 校准文件合并与导入
